...
var actions = {
	...
	'close-tab': function (id, call) {
		chrome.tabs.get(contents[id].detail.tabId, function (tab) {
			if (tab.pinned) call(); else if (settings.closeLastBlock) {
	...
	'undo-close': function (id, call) {
		if (pluginport) {
			pluginport.postMessage(JSON.stringify({ sendkey: { keys: ["t"], mods: [navMac ? "alt" : "ctrl", "shift"] } })); call();
		} else {
		...
		chrome.windows.getLastFocused(function (win) {
			if (!maximizeXWindow[win.id]) {
				...
				chrome.windows.update(win.id, {
					left: (screen.availWidth > screen.availHeight
				...
				chrome.windows.update(win.id, { left: dim.left, top: dim.top, width: dim.width, height: dim.height }, function () {
					setTimeout(function () {
						chrome.windows.update(win.id, { left: dim.left, top: dim.top, width: dim.width, height: dim.height }, call);
					}, 10);
				});
			}
		});
			...
			chrome.windows.update(mwin.id, { left: screen.availWidth - 10, top: screen.availHeight - 10, width: 10, height: 10 }, function () {
				setTimeout(function () {
					chrome.windows.update(mwin.id, { left: screen.availWidth - 10, top: screen.availHeight - 10, width: 10, height: 10 }, function () {
						...
						if (i < 0) call(); else focusWindow(focusedWindows[i], call);
					});
				}, 10);
			});
		});
	...
	'zoom-in': function (id, call) {
		if (pluginport) { pluginport.postMessage(JSON.stringify({ sendkey: { keys: ["+"], mods: [navMac ? "alt" : "ctrl"] } })); call(); }
	...
	'zoom-zero': function (id, call) {
		if (pluginport) { pluginport.postMessage(JSON.stringify({ sendkey: { keys: ["0"], mods: [navMac ? "alt" : "ctrl"] } })); call(); }
	...
	'zoom-img-in': function (id, call, a) {
		for (var i = 0; i < a.images.length; i++) {
			runJS(id, "var img = $('img[gestureid=" + a.images[i].gestureid + "]'); if(img) { if(!img.attr('origsize')) img.attr('origsize', img.width()+'x'+img.height()); img.css({'width':img.width()*1.2, 'height':img.height()*1.2}); }");
		} call();
	},
	...
	'zoom-img-zero': function (id, call, a) {
		for (var i = 0; i < a.images.length; i++) {
			runJS(id, "var img = $('img[gestureid=" + a.images[i].gestureid + "]'); if(img && img.attr('origsize')) { var size = img.attr('origsize').split('x'); img.css({'width':size[0]+'px', 'height':size[1]+'px'}); }");
		} call();
	},
	...
	'hide-image': function (id, call, a) {
		for (var i = 0; i < a.images.length; i++) {
			runJS(id, "var img = $('img[gestureid=" + a.images[i].gestureid + "]'); if(img) { img.css({'display':'none'}); }");
		} call();
	},
