...
var getText = function (name, param) {
	if (name.substr(0, 13) == "action_custom") {
		...
		if (bg.customActions[parts[1]]) return bg.customActions[parts[1]][parts.length == 3 ? "descrip" : "title"];
...
$("body").append($('<div>').attr('id', 'contents-contain').append($('<div>').attr('id', 'contents').append($('<div>').attr('class', 'title').text(getText("options_contents_actions"))).append($('<div>').attr('id', 'contentsactions')).append($('<div>').attr('class', 'title').text(getText("options_contents_settings"))).append($('<div>').attr('id', 'contentssettings')).append($("<ul>").append($('<li id="translatemess">Help <a id="translatelink" href="translate.html">Translate SG</a></li>')).append($('<li id="supportmess">Find a bug? <a id="supportlink" href="http://code.google.com/p/smoothgestures-chromium/issues/list">Report it</a></li>'))))).append($('<h1 id="optionstitle"><img src="im/icon48.png"/> ' + getText("name") + ': ' + getText("options_title") + '</h1>')).append($('<div>').attr('id', 'addgesture').append($("<div>").attr("id", "addgesturebutton").html("<span>+</span> " + chrome.i18n.getMessage("options_button_startaddgesture")).attr("tabindex", 0).click(function () {
	addGesture(null);
})))

if (bg.settings.sendStats == undefined) $("#optionstitle").after($('<div>').attr('id', 'note_sendStats').attr('class', 'note green').append($('<div>').attr("class", 'close').attr("tabindex", 0).html('&times;').click(function () {
...
if (bg.settings.hidePageAction == undefined) $("#optionstitle").after($('<div>').attr('id', 'note_hidePageAction').attr('class', 'note green').append($('<div>').attr("class", 'close').attr("tabindex", 0).html('&times;').click(function () {
...
if (!bg.settings.hideNoteTranslate && ['en', 'en-US', 'en-GB'].indexOf(navigator.language) == -1) $("#optionstitle").after($('<div>').attr('id', 'note_translate').attr('class', 'note green').append($('<div>').attr("class", 'close').attr("tabindex", 0).html('&times;').click(function () {
	$("#note_translate").remove();
	bg.settings.hideNoteTranslate = true;
	bg.saveOptions(bg.profile);
})).append($('<p>').html(getText("options_note_translate", "<a href='/translate.html'>" + getText("options_note_translate_link") + "</a>"))).append($("<div>").attr("class", "clearall")))

if (!bg.settings.hideNotePrint) $("#optionstitle").after($('<div>').attr('id', 'note_print').attr('class', 'note green').append($('<div>').attr("class", 'close').attr("tabindex", 0).html('&times;').click(function () {
	$("#note_print").remove();
...
if (!bg.settings.hideNoteWelcome) $("#optionstitle").after($('<div>').attr('id', 'note_welcome').attr('class', 'note clear').append($('<div>').attr("class", 'close').attr("tabindex", 0).html('&times;').click(function () {
	$("#note_welcome").remove();
...
if (!bg.settings.hideNoteRemindRate /* && new Date() - new Date(bg.getValue('date_first')) > 30*24*60*60*1000*/) {
	$("#optionstitle").after($('<div>').attr('id', 'note_remindrate').attr('class', 'note green').append($('<div>').attr("class", 'close').attr("tabindex", 0).html('&times;').click(function () {
		$("#note_remindrate").remove();
		bg.settings.hideNoteRemindRate = true;
		bg.saveOptions(bg.profile);
	})).append($('<p>').html(getText("options_note_remindrate", "<a href='https://chrome.google.com/webstore/detail/lfkgmnnajiljnolcgolmmgnecgldgeld'>" + getText("options_note_remindrate_link") + "</a>"))).append($("<div>").attr("class", "clearall")))
}

if (bg.settings.showNoteUpdated) {
	$("#optionstitle").after($('<div>').attr('id', 'note_updated').attr('class', 'note green').append($('<div>').attr("class", 'close').attr("tabindex", 0).html('&times;').click(function () {
		$("#note_updated").remove();
	})).append($('<p>').text(getText("options_note_updated", bg.extVersion))).append($("<div>").attr("class", "clearall")))
...
var generateOptions = function () {
	...
	for (cat in bg.categories)
		root.append(generateTable(cat));

	root.appendTo("body");

	for (gesture in bg.gestures)
		displayGesture(gesture);

	root.append($("<div>").attr("class", "footer").text("You have gestured approximately " + ("" + ((bg.log.line ? bg.log.line.distance : 0) * .000254)).substr(0, 8) + " meters."));
...
var generateTable = function (cat) {
	var table = $("<div>").attr("class", 'optiontable').attr("id", cat).append($("<a>").attr("name", cat)).append($("<div>").attr("class", 'tabletitle').text(getText(cat)));
	if (bg.categories[cat].actions) {
		...
		for (i = 0; i < bg.categories[cat].actions.length; i++) {
			var t = empty;
			for (g in bg.gestures) if (bg.gestures[g] == bg.categories[cat].actions[i]) {
				t = table;
				break;
			}
		...
		$(".tabletitle", table).append($("<div>").attr("id", "addactionbutton").html("<span>+</span> " + chrome.i18n.getMessage("options_button_addcustomaction")).attr("tabindex", 0).click(function () {
			addCustom();
		}))
		...
		var num = 0;
		table = $("<div>").attr("class", 'optiontable').append($("<a>").attr("name", cat));
		...
		$("#contentssettings").append($("<div>").append($("<a href='#'>").click(function () {
			scrollto(cat);
			return false;
		}).text(getText(cat))));
		table.append(settingsGroup(getText("setting_group_newtab"), "showNewTabSettings", $("<div>").append(rowNewTabUrl()).append(rowNewTabRight()).append(rowNewTabLinkRight()))).append(settingsGroup(getText("setting_group_trail"), "showTrailSettings", $("<div>").append(rowTrailBlock()).append(rowTrailColor()).append(rowTrailWidth()))).append(settingsGroup(getText("setting_group_context"), "showContextSettings", $("<div>").append(rowContextOnLink()))).append(settingsGroup(getText("setting_group_assorted"), "showAssortedSettings", $("<div>").append(rowCloseLastBlock()).append(rowSelectToLink()).append(rowBlacklist()))).append(rowHidePageAction()).append(rowMarketingAction()).append(rowSendStats()).append(rowHoldButton()).append(rowReset()).append(rowImport())
	}
	return table;
}
var actionGroup = function (title, label, show, rows) {
	return $("<div>").attr("class", "rowgroup").append($("<div>").attr("class", "actiongrouptitle").text(title).append($("<a>").attr("href", "#").text(!bg.settings['hide-' + label] ? getText("setting_group_hide") : getText("setting_group_show")).click(function () {
		...
		return false;
	}))).append(rows.attr("id", label + "Group").attr("class", "grouprows").css({
		"display": (!bg.settings['hide-' + label] ? "block" : "none")
	}))
}
var generateGRow = function (action) {
	return $("<div>").attr("class", 'actionrow').append($("<div>").attr("id", 'action-' + action).attr("class", "gestureset")).append($("<div>").attr("class", "gesture-add").attr("tabindex", 0).text("+").click(function () {
		addGesture(action)
	})).append(bg.contexts[action] ? $("<img>").attr("class", "actioncontext").attr("src", "im/icon-" + (bg.contexts[action] == "l" ? "link" : (bg.contexts[action] == "i" ? "image" : (bg.contexts[action] == "s" ? "selection" : ""))) + ".png") : null).append($("<div>").attr("class", "actiontitle").text(getText('action_' + action))).append($("<div>").attr("class", "actiondescrip").text(getText('descrip_' + action))).append($("<div>").attr("class", "clearall"));
}
var generateExternalRow = function (action, title, descrip) {
	return $("<div>").attr("class", 'actionrow').append($("<div>").attr("id", 'action-' + action).attr("class", "gestureset")).append($("<div>").attr("class", "gesture-add").attr("tabindex", 0).text("+").click(function () {
		addGesture(action)
	})).append($("<div>").attr("class", "actiontitle").text(title)).append($("<div>").attr("class", "actiondescrip").text(descrip)).append($("<div>").attr("class", "clearall"));
}
...
var generateCustomRow = function (id) {
	return $("<div>").attr("class", 'actionrow').append($("<div>").attr("id", 'action-' + id).attr("class", "gestureset")).append($("<div>").attr("class", "gesture-add").attr("tabindex", 0).text("+").click(function () {
		addGesture(id)
	})).append($("<div>").attr("class", "actiontitle").text(bg.customActions[id].title).append($("<span>").attr("class", "editcustom").attr("tabindex", 0).text("edit").click(function () {
		editCustom(id)
	})).append($("<span>").attr("class", "delcustom").attr("tabindex", 0).html("&times;").click(function () {
		delCustom(id)
	}))).append($("<div>").attr("class", "actiondescrip").text(bg.customActions[id].descrip)).append($("<div>").attr("class", "clearall"));
}
var editCustom = function (id) {
	var a = bg.customActions[id];
	var row = $("#action-" + id).parent().css({
		"display": "none"
	});
	var edit = $("<div>").attr("class", "actionrow").attr("id", "edit-" + id).append($("<div>").attr("class", "savecustom").attr("tabindex", 0).text("save").click(function () {
		saveCustom(id);
	...
	})).append($("<select>").attr("class", "cxtcustom").append($("<option>").text("starting anywhere").val("")).append($("<option>").text(chrome.i18n.getMessage("context_on_link")).val("l")).append($("<option>").text(chrome.i18n.getMessage("context_on_image")).val("i")).append($("<option>").text(chrome.i18n.getMessage("context_with_selection")).val("s"))).append($("<select>").attr("class", "envcustom").append($("<option>").text("context").attr("disabled", "disabled")).append($("<option>").text("page").val("page")).append($("<option>").text("extension").val("ext"))).append($("<div>").attr("class", "sharecustom").append($("<input type=checkbox>").attr("id", "share-" + id).attr("checked", a.share)).append($("<label>").attr("for", "share-" + id).text("share"))).append($("<input type=text>").attr("class", "titlecustom").val(a.title)).append($("<input type=text>").attr("class", "descripcustom").val(a.descrip)).append($("<textarea>").attr("class", "codecustom").text(a.code)).insertAfter(row)
	$(".cxtcustom", edit).val(a.context);
	$(".envcustom", edit).val(a.env);
}
var saveCustom = function (id) {
	var a = bg.customActions[id];
	$("#action-" + id).parent().css({
		"display": "block"
	});
	var edit = $("#edit-" + id);
...
var settingsGroup = function (title, label, rows) {
	return $("<div>").attr("class", "rowgroup").append($("<div>").attr("class", "settinggrouptitle").text(title).append($("<a>").attr("href", "#").text(bg.settings[label] ? getText("setting_group_hide") : getText("setting_group_show")).click(function () {
		...
		"display": (bg.settings[label] ? "block" : "none")
	}))
}
...
var rowReset = function () {
	return $('<div>').attr('class', "settingrow").append($("<div>").attr("class", "settingcontrol").append($("<input>").attr("type", "button").val(getText("setting_button_reset")).css({
		'min-width': '5em',
		'font-weight': 'bold'
	}).click(doReset))).append($("<div>").attr("class", "settingtitle").text(getText("setting_reset"))).append($("<div>").attr("class", "settingdescrip").text(getText("setting_reset_descrip")))
}
...
var rowImport = function () {
	return $('<div>').attr('class', "settingrow").append($("<div>").attr("class", "settingcontrol").append($("<div>").attr("class", "filespoof").append($("<input>").attr("type", "file").change(doImport)).append($("<input>").attr("type", "button").attr("tabindex", -1).val(getText("setting_button_import"))))).append($("<div>").attr("class", "settingcontrol").append($("<input>").attr("type", "button").val(getText("setting_button_export")).css({
		'min-width': '5em',
		'font-weight': 'bold'
	}).click(function (t) {
		chrome.tabs.create({
			url: "Smooth_Gestures_Settings.html"
		});
	...
	reader.onload = function (e) {
		...
		try {
			...
			if (importJSON.title !== "Smooth Gestures Settings") {
				alert("The setting data is not correct.");
				return;
			}
		} catch (e) {
			alert('Cannot import the setting: ' + e.message);
			console.error(e);
			return;
		}
...
var rowContextOnLink = function () {
	return $('<div>').attr("class", "settingrow").append($("<div>").attr("class", "settingcontrol").append($("<select>").attr("id", "contextOnLink").change(updateContextOnLink).append($("<option>").val("1").text(getText("setting_button_on"))).append($("<option>").val("0").text(getText("setting_button_off"))))).append($("<div>").attr("class", "settingtitle").text(getText("setting_force_context"))).append($("<div>").attr("class", "settingdescrip").text(getText("setting_force_context_descrip")))
}
...
var rowHoldButton = function () {
	return $('<div>').attr("class", "settingrow").append($("<div>").attr("class", "settingcontrol").append($("<select>").attr("id", "holdButton").change(updateHoldButton).append($("<option>").val("-1").text(getText("setting_disabled"))).append($("<option>").val("0").text(getText("setting_button_left"))).append($("<option>").val("1").text(getText("setting_button_middle"))).append($("<option>").val("2").text(getText("setting_button_right"))))).append($("<div>").attr("class", "settingtitle").text(getText("setting_hold_button"))).append($("<div>").attr("class", "settingdescrip").text(getText("setting_hold_button_descrip")))
}
...
var rowTrailColor = function () {
	...
	}).change(updateTrailColor))).append($("<div>").attr("id", "trailColorA").append($("<input>").attr({
		type: "range",
		min: 0,
		max: 1,
		step: .01,
...
var rowNewTabUrl = function () {
	return $('<div>').attr("class", "settingrow").append($("<div>").attr("class", "settingcontrol").append($("<select>").attr("id", "newTabUrl").change(updateNewTabUrl).append($("<option>").val("chrome://newtab/").text("New Tab page")).append($("<option>").val("homepage").text("Home page")).append($("<option>").val("custom").text("Custom page"))).append($("<input>").attr("id", "newTabUrlCustom").attr("type", "text").css({
		...
		"border": "1px solid gray",
		"margin": "0 .8em"
	})).append($("<input>").attr("type", "button").val(getText("setting_button_update")).click(updateNewTabUrl))).append($("<div>").attr("class", "settingtitle").text(getText("setting_newtab_url"))).append($("<div>").attr("class", "settingdescrip").text(getText("setting_newtab_url_descrip"))).append($("<div>").attr("class", "clearall"))
}
...
var rowNewTabRight = function () {
	return $('<div>').attr("class", "settingrow").append($("<div>").attr("class", "settingcontrol").append($("<select>").attr("id", "newTabRight").change(updateNewTabRight).append($("<option>").val("1").text(getText("setting_button_on"))).append($("<option>").val("0").text(getText("setting_button_off"))))).append($("<div>").attr("class", "settingtitle").text(getText("setting_newtab_right"))).append($("<div>").attr("class", "settingdescrip").text(getText("setting_newtab_right_descrip")))
}
...
var rowTrailBlock = function () {
	return $('<div>').attr("class", "settingrow").append($("<div>").attr("class", "settingcontrol").append($("<select>").attr("id", "trailBlock").change(updateTrailBlock).append($("<option>").val("1").text(getText("setting_button_on"))).append($("<option>").val("0").text(getText("setting_button_off"))))).append($("<div>").attr("class", "settingtitle").text(getText("setting_trail_draw"))).append($("<div>").attr("class", "settingdescrip").text(getText("setting_trail_draw_descrip")))
}
...
var rowCloseLastBlock = function () {
	return $('<div>').attr("class", "settingrow").append($("<div>").attr("class", "settingcontrol").append($("<select>").attr("id", "closeLastBlock").change(updateCloseLastBlock).append($("<option>").val("1").text(getText("setting_button_on"))).append($("<option>").val("0").text(getText("setting_button_off"))))).append($("<div>").attr("class", "settingtitle").text(getText("setting_closelastblock"))).append($("<div>").attr("class", "settingdescrip").text(getText("setting_closelastblock_descrip")))
}
...
var rowSelectToLink = function () {
	return $('<div>').attr("class", "settingrow").append($("<div>").attr("class", "settingcontrol").append($("<select>").attr("id", "selectToLink").change(updateSelectToLink).append($("<option>").val("1").text(getText("setting_button_on"))).append($("<option>").val("0").text(getText("setting_button_off"))))).append($("<div>").attr("class", "settingtitle").text(getText("setting_selecttolink"))).append($("<div>").attr("class", "settingdescrip").text(getText("setting_selecttolink_descrip")))
}
...
var rowHidePageAction = function () {
	return $('<div>').attr("class", "settingrow").append($("<div>").attr("class", "settingcontrol").append($("<select>").attr("id", "hidePageAction").change(updateHidePageAction).append($("<option>").val("1").text(getText("setting_button_on"))).append($("<option>").val("0").text(getText("setting_button_off"))))).append($("<div>").attr("class", "settingtitle").text(getText("setting_hidepageaction"))).append($("<div>").attr("class", "settingdescrip").text(getText("setting_hidepageaction_descrip")))
}
...
var rowMarketingAction = function () {
	return $('<div>').attr("class", "settingrow").append($("<div>").attr("class", "settingcontrol").append($("<select>").attr("id", "hideMarketingAction").change(updateMarketingAction).append($("<option>").val("1").text(getText("setting_button_on"))).append($("<option>").val("0").text("No, I'd rather you starve")))).append($("<div>").attr("class", "settingtitle").text("Enable Ads")).append($("<div>").attr("class", "settingdescrip").text("SmoothGestures is ad supported, the application includes advertising on select domains and is transparent to you the user. Disabling the setting removes no functionality, but it does make it financially difficult to maintain the code... geek's gotta eat. :-)"))
}
...
var rowBlacklist = function () {
	return $('<div>').attr("class", "settingrow").append($("<div>").attr("class", "settingcontrol").append($("<input>").attr("type", "button").val(getText("setting_button_update")).click(updateBlacklist))).append($("<div>").attr("class", "settingtitle").text(getText("setting_blacklist"))).append($("<div>").attr("class", "settingdescrip").text(getText("setting_blacklist_descrip"))).append($("<textarea>").attr("id", "blacklist").css({
		"width": "100%",
		"margin-top": ".5em",
		"height": "3em",
...
var displayGesture = function (gesture) {
	$('#action-' + bg.gestures[gesture]).append($('<div>').attr("id", 'gesture-' + gesture).attr("class", 'gesture').append($('<div>').attr("class", 'gesture-remove').attr("tabindex", 0).html('&times;').click(function () {
...
var addGesture = function (action) {
	...
	})).append($("<div>").attr("class", "canvastitle").text(getText("options_addgesture_title", getText('action_' + action)))).append($("<div>").attr("id", "canvasdescrip").attr("class", "canvasdescrip").append($("<div>").text(getText("options_addgesture_instruct_1"))).append($("<ul>").css({
		"margin": "0"
	...
	var receiveGesture = function (gesture) {
		...
		$("<div>").attr("id", "nowwhat").css({
		...
		})).append(action ? $("<input>").attr("type", "button").val(bg.gestures[gesture] ? getText("options_button_overwrite") : getText("options_button_addgesture")).css({
			"font-size": "1.1em",
			"float": "right",
			"min-width": "10em",
			"margin-right": "4em"
			...
			$(this).append($("<option>").attr("disabled", "disabled").text(bg.gestures[gesture] ? getText("options_button_overwrite") : getText("options_button_addgesture")));
		...
		if (bg.gestures[gesture]) {
			$("#nowwhat").prepend($("<div>").css({
				"text-align": "center",
				"font-size": "1.1em",
				"margin-bottom": ".5em",
				"color": "red"
			}).text(getText("options_addgesture_overwrite", getText('action_' + bg.gestures[gesture]))).appendTo(nowwhat))
		}

		$(window.SG.drawGesture(gesture, window.innerWidth * .8 / 2, window.innerHeight * .8 / 2)).attr("id", "gcanvas").css({
			"display": "block",
			"margin": "0 auto"
		}).appendTo(".drawingcanvas")
	}
...
var placeContents = function () {
	...
	element.css("top", (window.innerHeight <= element.height() ? 0 : (window.innerHeight >= 400 + element.height() ? 200 : (window.innerHeight - element.height()) / 2)));
}
placeContents();
